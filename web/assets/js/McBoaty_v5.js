...
if (!window.TBG.McBoaty_v5) {
	window.TBG.McBoaty_v5 = {
		init: false,
		clean: false,
        params: {
            denoise: {
...
export const McBoatyWidgets = {
    ...
    WRAPPER: (key, index, prompt, tile, denoise, node) => {
        ...
        textarea.addEventListener('focusout', async function() {
            ...
            if(window.TBG.McBoaty_v5.message.prompts[index] != this.value) {
                window.TBG.McBoaty_v5.message.prompts[index] = this.value;
                const res = await (await fetch(`/TBG/McBoaty/v5/set_prompt?index=${index}&prompt=${this.value}&node=${this.dataNodeId}&clientId=${api.clientId}`)).json();
                const nodeWidget = TBGUpscalerTileNodeWidget.getByName(node, 'requeue');
                TBGUpscalerTileNodeWidget.setValue(node, 'requeue', ++nodeWidget.value);
            }
        });
        ...
        input.addEventListener('focusout', async function() {
            ...
            if(window.TBG.McBoaty_v5.message.denoises[index] != this.value) {
                window.TBG.McBoaty_v5.message.denoises[index] = this.value;
                const res = await (await fetch(`/TBG/McBoaty/v5/set_denoise?index=${index}&denoise=${this.value}&node=${this.dataNodeId}&clientId=${api.clientId}`)).json();
                const nodeWidget = TBGUpscalerTileNodeWidget.getByName(node, 'requeue');
                TBGUpscalerTileNodeWidget.setValue(node, 'requeue', ++nodeWidget.value);
            }
        });
...
class MaraScottMcBoatyNodePrompter {
	...
	static async clean(node) {
		...
		window.TBG.McBoaty_v5.clean = false

        const cleanedLabel = " ... cleaned"
        const nodeTitle = node.title
		node.title = nodeTitle + cleanedLabel
        
        const res_prompts = await (await fetch(`/TBG/McBoaty/v5/get_input_prompts?node=${node.id}`)).json();
        const res_denoises = await (await fetch(`/TBG/McBoaty/v5/get_input_denoises?node=${node.id}`)).json();

        window.TBG.McBoaty_v5.message.prompts = window.TBG.McBoaty_v5.inputs.prompts = res_prompts.prompts_in;
        window.TBG.McBoaty_v5.inputs.denoises = window.TBG.McBoaty_v5.message.denoises = res_denoises.denoises_in;
        window.TBG.McBoaty_v5.message.tiles = window.TBG.McBoaty_v5.inputs.tiles;
        TBGUpscalerTileNodeWidget.setValue(node, TBGUpscalerTileNodeWidget.INDEX.name, TBGUpscalerTileNodeWidget.INDEX.default);
        TBGUpscalerTileNodeWidget.setValue(node, TBGUpscalerTileNodeWidget.PROMPT.name, TBGUpscalerTileNodeWidget.PROMPT.default);
        TBGUpscalerTileNodeWidget.setValue(node, TBGUpscalerTileNodeWidget.DENOISE.name, TBGUpscalerTileNodeWidget.DENOISE.default);
        TBGUpscalerTileNodeWidget.setValue(node, 'requeue', 0);
        ...
        node.widgets = node.widgets.filter(widget => {
            ...
            if (widget.type == "customtext") {
                ...
                if ( textarea != null) {
                    ...
                    const value = window.TBG.McBoaty_v5.inputs.prompts[realIndexValue];
                    TBGUpscalerTileNodeWidget.setValue(node, widget.name, value);
...
class TBGUpscalerTileNodeWidget {
	...
	static setDenoiseInput(node) {
		...
		if (nodeWidget == undefined) {
			...
			node.addWidget(
				...
				{
					"values": window.TBG.McBoaty_v5.params.denoise.values
				}
			)
			this.setValue(node, this.DENOISE.name, this.DENOISE.default)
		}
    ...
    static setPrompterInputs(node) {
        ...
        for (const [index] of window.TBG.McBoaty_v5.message.prompts.entries()) {
            if (index_list.length == 0 || index_filtered.indexOf(index) > -1) {
                const w = McBoatyWidgets.WRAPPER("tile "+index, index, window.TBG.McBoaty_v5.message.prompts[index], window.TBG.McBoaty_v5.message.tiles[index], window.TBG.McBoaty_v5.message.denoises[index], node);
            }
        }
...
window.TBG.McBoaty_v5.params.denoise.values = TBGUpscalerTileNodeWidget.DENOISE.values;
for (let i = TBGUpscalerTileNodeWidget.DENOISE.min; i <= TBGUpscalerTileNodeWidget.DENOISE.max; i = parseFloat((i + TBGUpscalerTileNodeWidget.DENOISE.step).toFixed(2))) {
    window.TBG.McBoaty_v5.params.denoise.values.push(i.toFixed(2));
}
...
class McBoaty_v5 {
	...
	addStatusTagHandler(nodeType) {
		...
		api.addEventListener("TBG/McBoaty_v5/update_status", ({ detail }) => {
	...
	async beforeRegisterNodeDef(nodeType, nodeData, app) {
		...
		if (nodeData.name === "TBG_TilePrompter_v1") {
			...
			nodeType.prototype.onExecuted = function (message) {
                ...
                window.TBG.McBoaty_v5.inputs.prompts = message.prompts_in || [];
                window.TBG.McBoaty_v5.message.prompts = message.prompts_out || [];
                window.TBG.McBoaty_v5.inputs.denoises = message.denoises_in || [];
                window.TBG.McBoaty_v5.message.denoises = message.denoises_out || [];
                window.TBG.McBoaty_v5.inputs.tiles = window.TBG.McBoaty_v5.message.tiles = message.tiles || [];
			...
			nodeType.prototype.getExtraMenuOptions = function (_, options) {
				...
				if (img) {
					...
					options.splice(pos, 0, {
						...
						callback: async () => {
							let src = img.src;
							src = src.replace("/view?", `/TBG/McBoaty_v5/tile_prompt?node=${this.id}&clientId=${api.clientId}&`);
